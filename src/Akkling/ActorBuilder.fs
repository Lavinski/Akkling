...
type ActorBuilder() =
    ...
    member __.Bind(_ : IO<'In>, continuation : 'In -> #Effect<'In>) : #Effect<'Message> = 
        Become(fun message -> 
            continuation message :> Effect<_>)
    
    // Effects
    member this.Bind(behavior : #Effect<'In>, continuation : #Effect<'In> -> #Effect<'In>) : #Effect<'In> = 
        let asd = behavior :> Effect<'In>
        match asd with
        | :? Become<'In> as become -> 
            Become<'In>(fun message -> 
                let m = become.Next message :?> #Effect<'In>
                let cont = (fun (x:#Effect<'In>) -> continuation x)
                let result = this.Bind(m, cont)
                result :> _
            ) :> obj :?> _
        | returned -> continuation (returned :?> _) :> obj :?> _
